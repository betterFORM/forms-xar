...
declare function local:handleFile($uri as xs:string, $contextPath as xs:string, $path as xs:string, $ajaxFunction as xs:string, $fileName as xs:string, $shorten as xs:string) {
    ...
    let $filePath := if(contains($uri,"rest"))
    then ( functx:substring-before-last($uri, 'rest/db/apps/betterform/modules'))
    else ( functx:substring-before-last($uri, 'apps/betterform/modules'))
    ...
    return
        if (fn:not(fn:contains($ignores, $fileName))) then (
            <div class="file">
                ...
                <a class="sourceLink" title="view" href="{fn:concat($filePath, $path, '/', $fileLink, '?__bf:source=true')}" target="_blank">source</a>
